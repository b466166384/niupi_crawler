...
# 从 1 开始循环便利总页数到 a_pager 范围 [1,a_pager+1]   5
for i in range(58, a_pager + 1):
    # 当为 1 则拼接 a_url 对应的链接赋值给 a_pager_url ，将 a_content 赋值给 a_pager_url_content
    if i == 1:
        a_pager_url = a_url
    ...
    b_count = len(b_urls)
    print(f'首页-第{i}页，网址：{a_pager_url}')
    if(i == 61):
        print(f'首页-第{i}页，退出了')
        sys.exit()
    # 从 0 开始循环便利总数到 b_count 范围 [0,b_count]
    for j in range(b_count):
        ...
        for k in range(1, b_pager + 1):
            ...
            # 从 a_b_pager_url_content 中获取 <div class="pt-4"></div> 元素并从中匹配 <div class="my-1"></div> 并从中匹配全部的 <img class="block my-1" src="" title="" alt=""> 的图像资源并提取其中的src链接为 c_url 并统计总数量 c_count
            if a_b_pager_url_content is not None:
                ...
                print(c_urls)
                save_list_to_txt(c_urls, "../fuligirl.top爬美图.txt")
                c_count = len(c_urls)
                ...
                print("a_b_pager_url_content 为 None，请检查赋值。")
        # 返回上一层目录
        os.chdir(a_name)
