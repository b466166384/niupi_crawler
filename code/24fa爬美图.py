...
for i in range(6, a_pager + 1):
    if i == 1:
        ...
        b_pager_content = ""
        if not b_pager_content:
            with open('/PagerContent.txt', 'r', encoding='utf-8') as file:
                b_pager_content = file.read()
        # 当不为 1 则处理 a_url 将结尾类似 c49.aspx 的部分中 .aspx 去掉，得到 a_url_cut 用于后续拼接 
    ...
    b_count = len(b_links)
    print(f'首页-第{i}页，网址：{a_url_pager}')
    # 遍历每个链接
    for j in range(0,b_count):
        ...
        for j in range(1, b_pager + 1):
            ...
            save_list_to_txt(c_text, "../24fa爬美图.txt")
            c_count = len(c_text)
