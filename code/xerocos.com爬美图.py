...
def request_with_retry(url, max_retries=5, timeout=10, retry_delay=2):
    retries = 0
    while retries < max_retries:
        try:
            # 发送请求并设置超时时间
            ...
            if response.status_code == 200:
                ...
                logging.warning(f"Request to {url} failed with status code {response.status_code}, retrying...")
        except requests.exceptions.Timeout:
            logging.warning(f"Request to {url} timed out, retrying...")
        except requests.exceptions.RequestException as e:
            logging.warning(f"Request to {url} failed with {str(e)}, retrying...")
        retries += 1
    ...
    for j in range(b_count):
        ...
        for k in range(1, b_pager + 1):
            ...
            for l in range(c_count):
                # 将 b_name 修改为 a_name
                c_name = os.path.join(a_name, c_names[l].replace(' ','-').replace(":"," ").replace('-–-','-').replace('--','-').replace('#','').replace('@','').lstrip('-').rstrip('-'))
                # 创建对应的 c_name 目录并进入目录，如果存在则不创建目录直接进入目录
                ...
                for m in range(1, c_pager + 1):
                    ...
                    save_list_to_txt(d_urls, "../xerocos.com爬美图.txt")
                    d_count = len(d_urls)
                    # 从 0 开始循环便利总数到 d_count 范围 [0,d_count]
