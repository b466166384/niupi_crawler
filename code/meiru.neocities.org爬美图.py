...
from save_list_to_txt import save_list_to_txt
...
for i in range(2, a_pager + 1):
    ...
    outer_divs = soup.find_all('div', class_='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 p-1')
    divs = outer_divs[0].find_all('div', attrs={"class": None})
    href_values = []
    ...
    for j in range(b_count):
        ...
        b_url = b_urls[j]
        print(f'b_url:{b_url}')
        b_content = request_with_retry("https://meiru.neocities.org/"+b_url)
        # 从 b_content 中获取链接总页数 b_pager
        soup = BeautifulSoup(b_content, 'html.parser')
        # last_page_link = int(soup.select_one('div#pagination div:last-child div:last-child a:nth-last-child(2)')['href'].split('/')[-2])
        last_page_link = 1

        d_url = soup.select('div#gallery img[src]')
        c_url = [img['src'] for img in d_url]
        print(f'c_url:{c_url}')
        print(c_url)
        # 假设 soup 是已经解析好的 BeautifulSoup 对象
        ...
        for k in range(1, b_pager + 1):
            ...
            new_urls = [url.replace('..', 'https://meiru.neocities.org/posts') for url in d_urls]
            d_urls = new_urls
            print(f'd_urls，网址：{d_urls}')
            d_names = [div.find('img', {'class': 'rounded-lg shadow-sm w-full h-72 md:h-96 object-cover'})['alt'] for div in divs]
            d_count = len(d_urls)
            print(f'c_b_pager_url第{k}页，网址：{c_b_pager_url}')
            print(f'd_count，网址：{d_count}')
            ...
            for l in range(d_count):
                ...
                save_list_to_txt(e_urls, "../meiru.neocities.org爬美图.txt")
                e_count = len(e_urls)
