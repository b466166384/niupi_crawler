...
if __name__ == '__main__':
    ...
    with sync_playwright() as p:
        try:
            ...
            # pic_detail_url_list = ['https://missav123.com/dm70/cn/ipzz-020']
            if pic_detail_url_list:
                detail_url = pic_detail_url_list[0]
                #设置chromedriver
                chrome_options = Options()
                chrome_options.binary_location = r'D:/sdk/chrome-win64/chrome.exe'
                selenium_browser = webdriver.Chrome(service=Service('D:/sdk/chromedriver-win64/chromedriver.exe'), options=chrome_options)
                selenium_browser.get(detail_url)
                sleep(2)
                # 提取页面内容
                page_content = selenium_browser.page_source
                parser = etree.HTMLParser(encoding="utf-8")
                detail_tree = etree.HTML(page_content, parser=parser)
                # 提取图片URL
